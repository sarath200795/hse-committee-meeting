...
const outFile = path.join(outDir, 'hse-committee-meeting.mp4')
const silentFile = path.join(outDir, 'hse-committee-meeting.silent.mp4')
...
const t0 = Date.now()
const rec = await startScreencast(page, path.join('/tmp', 'frames-hse-committee-meeting'))
initAudio(path.join('/tmp', 'audio-hse-committee-meeting'), t0)
...
if (res) {
  ...
  const wav = path.join('/tmp', 'audio-hse-committee-meeting', 'narration.wav')
